...
class ProcessingDataImage : AppCompatActivity() {
        ...
        call.enqueue(object : Callback<ServerResponse> {
            override fun onResponse(call: Call<ServerResponse>, response: Response<ServerResponse>) {
                if (response.isSuccessful && response.body() != null) {
                    ...
                    Toast.makeText(this@ProcessingDataImage, "Ошибка сервера", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<ServerResponse>, t: Throwable) {
                Toast.makeText(this@ProcessingDataImage, "Ошибка сети: ${t.message}", Toast.LENGTH_LONG).show()
            }
        })
    }

    private fun returnResult() {
        val resultIntent = Intent(this@ProcessingDataImage, ImageDisplayActivity::class.java).apply {
